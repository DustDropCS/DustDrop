...
function pickUpgradeRewardItem(selectedItem, chance) {
  ...
  const maxRewardPrice = inputPrice * (100 / chance);
  const candidates = availableSkins
    .filter((item) => {
      const price = Number(item.price);
      return Number.isFinite(price) && price > inputPrice && price <= maxRewardPrice;
    })
    .map((item) => ({
      item,
      distance: maxRewardPrice - Number(item.price || 0)
    }))
