...
const DEFAULT_CASE_SECTIONS = [
  {
    ...
    cases: [
      { id: 'blue-case', name: 'Синий Кейс', price: 2490, image: 'https://jabka.to/cdn/items/18866/large.webp' },
      { id: 'green-case', name: 'Зеленый Кейс', price: 2990, image: 'https://jabka.to/cdn/items/18593/large.webp' },
      { id: 'red-case', name: 'Красный Кейс', price: 3490, image: 'https://jabka.to/cdn/items/614/large.webp' },
      { id: 'pink-case', name: 'Розовый Кейс', price: 4290, image: 'https://jabka.to/cdn/items/20537/large.webp' },
      { id: 'rainbow-case', name: 'Радужный Кейс', price: 6990, image: 'https://i.ibb.co/3yJN2LqB/image-removebg-preview-63.png' }
    ]
  }
];
...
async function loadRemoteCaseSections() {
  ...
  if (remoteSections.length) {
    localStorage.setItem(CASE_SECTIONS_KEY, JSON.stringify(remoteSections));
    renderHomeCaseSections();
  }
}
...
const CASE_SKINS = [
  {
    name: 'Default Skin',
    image: REMOTE_SKIN_IMAGES.gray,
    rarity: 'gray',
    price: 0
  },
  {
    name: 'AK-47',
    image: REMOTE_SKIN_IMAGES.blue,
    rarity: 'blue'
  },
  {
    name: 'AWP',
    image: REMOTE_SKIN_IMAGES.green,
    rarity: 'green'
  },
  {
    name: 'M4A1-S',
    image: REMOTE_SKIN_IMAGES.red,
    rarity: 'red'
  },
  {
    name: 'USP-S',
    image: REMOTE_SKIN_IMAGES.purple,
    rarity: 'purple'
  },
  {
    name: 'Glock-18',
    image: REMOTE_SKIN_IMAGES.pink,
    rarity: 'pink'
  },
  {
    name: 'Desert Eagle',
    image: REMOTE_SKIN_IMAGES.gold,
    rarity: 'gold'
  }
];

let availableSkins = CASE_SKINS;
...
async function loadCaseContents(caseId) {
  if (!caseId || !supabaseClient) {
    activeCaseSkins = availableSkins;
...
function rollCase() {
  ...
  if (!activeCaseSkins.length && activeCaseId) {
    return;
  }
...
async function triggerCaseOpenAction() {
  ...
  if (supabaseClient && activeCaseId) {
    ...
    if (rewardError || !reward) {
      alert(rewardError?.message || 'Не удалось открыть кейс.');
      return;
    }
    ...
    saveActiveUser(user);
    pushLiveFeedItem(pendingCaseReward);
    addDroppedItemToInventory(pendingCaseReward);
    ...
    const currentBalance = clampBalance(user.balance);
      ...
      const profilePayload = existingProfileId
        ? {
          username,
      ...
      const insertResult = existingProfileId
        ? await supabaseClient.from('profiles').update(profilePayload).eq('id', existingProfileId)
...
async function refreshActiveUserFromSupabase() {
  ...
  let { data, error } = await supabaseClient
    .from('profiles')
    .select('username, role, balance, trade_url, inventory, opened_cases, upgrades, withdrawn_total, payout_system_enabled, payout_chance, case_payout_chance, upgrade_payout_chance, best_drop')
    .eq('id', user.id)
    .maybeSingle();
  ...
  if (error && /withdrawn_total/i.test(error.message || '')) {
    ({ data, error } = await supabaseClient
      .from('profiles')
      .select('username, role, balance, trade_url, inventory, opened_cases, upgrades, payout_system_enabled, payout_chance, case_payout_chance, upgrade_payout_chance, best_drop')
      .eq('id', user.id)
      .maybeSingle());
  }
...
async function sellAllItems() {
  ...
  const inventory = getInventoryByUser(user);
  const availableItems = inventory.filter((item) => !isWithdrawnItem(item));
  const totalValue = availableItems.reduce((sum, item) => sum + Number(item.price || 0), 0);
  if (!supabaseClient || !availableItems.length) {
    return;
  }

  const { data, error } = await supabaseClient.rpc('sell_all_items');
  if (error || !data?.length) {
    lastSupabaseError = error?.message || 'Не удалось продать предметы.';
    window.alert(lastSupabaseError);
    return;
  }

  user.balance = Number(data[0].balance || 0);
...
function pushLiveFeedItem(item) {
  ...
  const feed = [record, ...getLiveFeedItems()].slice(0, 30);
...
function getUpgradeInventoryItems() {
  const user = getActiveUser();
  return Array.isArray(user?.inventory) ? user.inventory.filter((item) => !isWithdrawnItem(item)) : [];
}
...
function syncUpgradeButtonState() {
  ...
  const hasReward = Boolean(selectedItem && getUpgradeRewardPreview(selectedItem));

  if (upgradeScreenEls.button) {
    upgradeScreenEls.button.disabled = !hasSelection || !hasReward || upgradeScreenSpinning;
  }
...
function pickUpgradeRewardItem(selectedItem, chance) {
  ...
  const candidates = availableSkins
    ...
    .map((item) => ({
      item,
      distance: Math.abs(Number(item.price || 0) - targetPrice)
    }))
    .sort((left, right) => left.distance - right.distance);
  if (!candidates.length) {
    return availableSkins.find((item) => Number.isFinite(Number(item.price)) && Number(item.price) > inputPrice) || null;
  }
...
function getUpgradeRewardPreview(selectedItem) {
  ...
  const automaticChance = inputPrice > 0 && rewardPrice > 0
    ? getCanonicalUpgradeChance((inputPrice / rewardPrice) * 100)
    : chance;
  if (upgradeScreenConfig.chance !== automaticChance || selectedUpgradeChance !== automaticChance) {
    upgradeScreenConfig.chance = automaticChance;
    selectedUpgradeChance = automaticChance;
    if (upgradeScreenEls.chanceElement) {
      upgradeScreenEls.chanceElement.textContent = `${formatUpgradeChance(automaticChance)}%`;
    }
    upgradeScreenEls.winArcs.forEach((arc) => {
      arc.setAttribute('stroke-dasharray', `${automaticChance} ${100 - automaticChance}`);
    });
  }
...
function getUpgradeStopAngle(payoutChance = upgradeScreenConfig.chance) {
  const effectiveChance = getUpgradePayoutChance(payoutChance);
  const winSize = effectiveChance * 3.6;
  const shouldWin = Math.random() * 100 < effectiveChance;
...
function runUpgradeAnimation() {
  ...
  upgradeScreenStopAngle = getUpgradeStopAngle(payoutChance);
