...
async function runUpgradeAnimation() {
  ...
  const { data: upgradeData, error: upgradeError } = await supabaseClient.rpc('upgrade_item', {
    selected_name: selectedItem.name,
    requested_chance: selectedUpgradeChance
  });
  const serverResult = Array.isArray(upgradeData) ? upgradeData[0] : upgradeData;
  if (upgradeError || !serverResult) {
    if (upgradeScreenEls.status) {
      upgradeScreenEls.status.textContent = upgradeError?.message || 'Не удалось выполнить апгрейд';
    }
    return;
  }

  if (serverResult.won && serverResult.name) {
    selectedUpgradeRewardItem = {
      name: serverResult.name,
      price: Number(serverResult.price || 0),
      rarity: serverResult.rarity || 'blue',
      image: resolveAssetPath(serverResult.image || getSkinImageByRarity(serverResult.rarity || 'blue'))
    };
  }
  ...
  upgradeScreenStopAngle = getUpgradeStopAngle(payoutChance, Boolean(serverResult.won));
  ...
  upgradeAnimationTimeoutId = window.setTimeout(async () => {
    upgradeAnimationTimeoutId = null;
    const win = Boolean(serverResult.won);
    upgradeScreenSpinning = false;
    setUpgradeControlsLocked(false);
