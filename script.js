...
function sellAllItems() {
  ...
  user.balance = Number(user.balance || 0) + totalValue;
...
function getUpgradeStopAngle(payoutChance = upgradeScreenConfig.chance) {
  ...
  const shouldWin = Math.random() * 100 < payoutChance;
...
function runUpgradeAnimation() {
  ...
  const user = getActiveUser();
  const payoutChance = getPlayerPayoutChance(user, getUpgradePayoutChance(upgradeScreenConfig.chance));
